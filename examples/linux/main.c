...
//    fclose(fp);
    ...
    uint32_t file_size = ftell(g_pInFile);
    rewind(g_pInFile);

//    const uint32_t start = 0x0000166e;
//
//    uint8_t ch = 0;
//    fseek(g_pInFile, start, SEEK_SET);
//    for (uint32_t i = start; i < 0x1b58; i++) {
//        fread(&ch, 1, 1, g_pInFile);
//        printf("0x%08x:  0x%02x  %03d %c\n", i, ch, ch, ch);
//    }
//
//    rewind(g_pInFile);
//    printf("\n\n");
//
//    fseek(g_pInFile, start, SEEK_SET);
//    for (uint32_t i = start; i < 0x1b58; i+=4) {
//        fread(&fourcc, 1, 4, g_pInFile);
...
//    uint32_t fourcc = 0;
    ...
    for (int k = 0; k < 5; k++) {
        ...
        for (uint32_t i = ftell(g_pInFile); i < file_size; i++) {
            fread(&chr, 1, 1, g_pInFile);
            if (chr == '0') {  // (0,0,d,c)
                fread(&chr, 1, 1, g_pInFile);
                i++;
        ...
        printf("pjpeg_decode_init ftell = %x\n", ftell(g_pInFile));
        printf("pjpeg_decode_init g_nInFileSize = %x\n", g_nInFileSize);
